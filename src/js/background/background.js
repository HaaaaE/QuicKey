	...
	tab)
{
	...
	return (tab?.url || tab?.pendingUrl || tab?.documentUrl || "").startsWith(k.PopupURL);
}
...
function isPopupTab(
	tab)
{
	return tab?.id === popupWindow.tabID || isPopupWindow(tab);
}
...
function getCachedTabData(
	tabs,
	sessions)
{
	return {
		tabs: filterPopupTabs(tabs),
		sessions,
		popupTabID: popupWindow.tabID
	};
}
...
//console.log("---- background: onConnect", port.name);
	...
	if (port.name === "popup" || port.name === "menu") {
		...
		const tabsPromise = cachedTabs || getBrowserTabs();
...
const storagePromise = prefetchedStoragePromise || storage.get()
		...
		Promise.all([tabsPromise, sessionsPromise, storagePromise, frecencyPromise])
			.then(([tabs, sessions, storageData, frecencyMap]) => {
				const cachedTabData = getCachedTabData(tabs, sessions);

				try {
					port.postMessage({
						message: "prefetchedData",
						tabs: cachedTabData.tabs,
						sessions: cachedTabData.sessions,
						popupTabID: cachedTabData.popupTabID,
						storageData,
						frecencyMap
					});
				} catch (e) {
...
chrome.runtime.onMessage.addListener(({message, ...payload}, sender, sendResponse) => {
	...
	if (handler) {
		...
		(async () => {
			...
			sendResponse(getCachedTabData(tabs, sessions));
		})();
		asyncResponse = true;
